      ...
      <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {chartType === 'sankey' && (
            <button
              onClick={() => {
                const sampleData = JSON.parse(TemplateService.generateTemplate('sankey', 'json'));
                onDataImported(sampleData);
              }}
              className="flex items-center gap-2 px-3 py-2 text-sm bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-md hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors"
            >
              <FileText size={14} />
              Carregar Exemplo
            </button>
